...
static void notify_adjustment_cb (theaterTimeEntry *self, GParamSpec *pspec, gpointer user_data);
static void changed_cb (GtkAdjustment *adjustment, theaterTimeEntry *self);

struct theaterTimeEntryPrivate {
	GtkAdjustment *adjustment;
	gulong adjustment_changed_signal;
};

G_DEFINE_TYPE (theaterTimeEntry, theater_time_entry, GTK_TYPE_SPIN_BUTTON)

static gint64
theater_string_to_time (const char *time_string)
{
...
static void
theater_time_entry_class_init (theaterTimeEntryClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (theaterTimeEntryPrivate));

	object_class->dispose = dispose;
}

static void
theater_time_entry_init (theaterTimeEntry *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, theater_TYPE_TIME_ENTRY, theaterTimeEntryPrivate);
...
{
	theaterTimeEntryPrivate *priv = theater_TIME_ENTRY (object)->priv;
	...
	G_OBJECT_CLASS (theater_time_entry_parent_class)->dispose (object);
}

GtkWidget *
theater_time_entry_new (GtkAdjustment *adjustment, gdouble climb_rate)
{
	return g_object_new (theater_TYPE_TIME_ENTRY,
...
output_cb (GtkSpinButton *self, gpointer user_data)
{
	...
	text = theater_time_to_string ((gint64) gtk_spin_button_get_value (self) * 1000, FALSE, FALSE);
...
input_cb (GtkSpinButton *self, gdouble *new_value, gpointer user_data)
{
	...
	val = theater_string_to_time (gtk_entry_get_text (GTK_ENTRY (self)));
	if (val == -1)
...
static void
notify_adjustment_cb (theaterTimeEntry *self, GParamSpec *pspec, gpointer user_data)
{
	theaterTimeEntryPrivate *priv = self->priv;
...
static void
changed_cb (GtkAdjustment *adjustment, theaterTimeEntry *self)
{
	gchar *time_string;
	guint upper, width;
	...
	time_string = theater_time_to_string (((gint64) upper) * 1000, FALSE, FALSE);
