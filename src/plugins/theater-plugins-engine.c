...
typedef struct _theaterPluginsEnginePrivate{
	PeasExtensionSet *activatable_extensions;
	theaterObject *theater;
	GSettings *settings;
	guint garbage_collect_id;
} _theaterPluginsEnginePrivate;

G_DEFINE_TYPE(theaterPluginsEngine, theater_plugins_engine, PEAS_TYPE_ENGINE)

static void theater_plugins_engine_dispose (GObject *object);

static gboolean
garbage_collect_cb (gpointer data)
{
	theaterPluginsEngine *engine = (theaterPluginsEngine *) data;
	peas_engine_garbage_collect (PEAS_ENGINE (engine));
	return TRUE;
}

static void
theater_plugins_engine_class_init (theaterPluginsEngineClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->dispose = theater_plugins_engine_dispose;
	g_type_class_add_private (klass, sizeof (theaterPluginsEnginePrivate));
}
...
theaterPluginsEngine *
theater_plugins_engine_get_default (theaterObject *theater)
{
	static theaterPluginsEngine *engine = NULL;
	char **paths;
	guint i;
	...
	g_return_val_if_fail (theater != NULL, NULL);
	...
	g_irepository_require (g_irepository_get_default (), "theater", theater_API_VERSION, 0, NULL);

	paths = theater_get_plugin_paths ();

	engine = theater_PLUGINS_ENGINE (g_object_new (theater_TYPE_PLUGINS_ENGINE,
						     NULL));
	for (i = 0; paths[i] != NULL; i++) {
		/* theater uses the libdir even for noarch data */
	...
	engine->priv->theater = g_object_ref (theater);
 ...
 * earlier than the rest of theater, so that (for example) they can display modal save dialogues and the like. */
void
theater_plugins_engine_shut_down (theaterPluginsEngine *self)
{
	theaterPluginsEnginePrivate *priv = self->priv;

	g_return_if_fail (theater_IS_PLUGINS_ENGINE (self));
...
static void
theater_plugins_engine_init (theaterPluginsEngine *engine)
{
	engine->priv = G_TYPE_INSTANCE_GET_PRIVATE (engine,
						    theater_TYPE_PLUGINS_ENGINE,
						    theaterPluginsEnginePrivate);

	engine->priv->settings = g_settings_new (theater_GSETTINGS_SCHEMA);

	engine->priv->garbage_collect_id = g_timeout_add_seconds_full (G_PRIORITY_LOW, 20, garbage_collect_cb, engine, NULL);
	g_source_set_name_by_id (engine->priv->garbage_collect_id, "[theater] garbage_collect_cb");
}

static void
theater_plugins_engine_dispose (GObject *object)
{
	theaterPluginsEngine *engine = theater_PLUGINS_ENGINE (object);

	if (engine->priv->activatable_extensions != NULL)
		theater_plugins_engine_shut_down (engine);
	...
	if (engine->priv->theater)
		g_object_unref (engine->priv->theater);
	engine->priv->theater = NULL;

	if (engine->priv->settings != NULL)
		g_object_unref (engine->priv->settings);
	engine->priv->settings = NULL;

	G_OBJECT_CLASS (theater_plugins_engine_parent_class)->dispose (object);
}
