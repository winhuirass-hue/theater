...
#define theater_TYPE_AUTOLOAD_SUBTITLES_PLUGIN	(theater_autoload_subtitles_plugin_get_type ())
#define theater_AUTOLOAD_SUBTITLES_PLUGIN(o)		(G_TYPE_CHECK_INSTANCE_CAST ((o), theater_TYPE_AUTOLOAD_SUBTITLES_PLUGIN, theaterAutoloadSubtitlesPlugin))
...
} theaterAutoloadSubtitlesPluginPrivate;

theater_PLUGIN_REGISTER(theater_TYPE_AUTOLOAD_SUBTITLES_PLUGIN, theaterAutoloadSubtitlesPlugin, theater_autoload_subtitles_plugin)
...
static gboolean
theater_uri_exists (const char *uri)
{
...
static char *
theater_uri_get_subtitle_for_uri (const char *uri)
{
	char *subtitle;
	guint len, i;
	gint suffix;
	...
	for (i = 0; i < G_N_ELEMENTS (subtitle_ext) ; i++) {
		...
		if (theater_uri_exists (subtitle))
			return subtitle;
...
static char *
theater_uri_get_subtitle_in_subdir (GFile *file, const char *subdir)
{
	...
	subtitle = theater_uri_get_subtitle_for_uri (full_path_str);
...
static char *
theater_uri_get_cached_subtitle_for_uri (const char *uri)
{
	...
	ret = theater_uri_get_subtitle_for_uri (fake_uri);
	g_free (fake_uri);
...
static char *
theater_uri_get_subtitle_uri (const char *uri)
{
	GFile *file;
	char *subtitle;
	...
	subtitle = theater_uri_get_cached_subtitle_for_uri (uri);
	if (subtitle != NULL) {
		g_object_unref (file);
		return subtitle;
	}
	...
	subtitle = theater_uri_get_subtitle_for_uri (uri);
	if (subtitle != NULL) {
		g_object_unref (file);
		return subtitle;
	}

	subtitle = theater_uri_get_subtitle_in_subdir (file, "subtitles");
	g_object_unref (file);
...
static char *
get_text_subtitle_cb (theaterObject                  *theater,
		      const char                   *mrl,
		      theaterAutoloadSubtitlesPlugin *pi)
{
	char *sub;
	...
	sub = theater_uri_get_subtitle_uri (mrl);
...
{
	theaterAutoloadSubtitlesPlugin *pi = theater_AUTOLOAD_SUBTITLES_PLUGIN (plugin);

	pi->priv->theater = g_object_ref (g_object_get_data (G_OBJECT (plugin), "object"));
	pi->priv->settings = g_settings_new ("org.gnome.theater");
	...
	pi->priv->signal_id = g_signal_connect (G_OBJECT (pi->priv->theater), "get-text-subtitle",
