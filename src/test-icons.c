	    ...
	    gpointer      user_data)
{
	...
	pixbuf = theater_grilo_get_thumbnail_finish (source_object, res, NULL);
	if (!pixbuf) {
...
set_icon_from_grl (GObject   *object,
		   GtkWidget *image)
{
	theater_grilo_get_thumbnail (object, NULL, icon_ready, image);
}
...
int main (int argc, char **argv)
{
	...
	gtk_image_set_from_pixbuf (GTK_IMAGE (images[i++]),
				   (GdkPixbuf *) theater_grilo_get_box_icon ());

	gtk_image_set_from_pixbuf (GTK_IMAGE (images[i++]),
				   (GdkPixbuf *) theater_grilo_get_channel_icon ());

	object = media_object ("file:///somewhere/over/the/rainbow.png");
	gtk_image_set_from_pixbuf (GTK_IMAGE (images[i++]),
				   theater_grilo_get_icon (GRL_MEDIA (object), &tmp));
	...
	gtk_image_set_from_pixbuf (GTK_IMAGE (images[i++]),
				   (GdkPixbuf *) theater_grilo_get_optical_icon ());
