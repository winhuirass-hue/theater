...
struct theaterOpenLocationPrivate
{
	GtkWidget *uri_container;
	GtkEntry *uri_entry;
};

#define theater_OPEN_LOCATION_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), theater_TYPE_OPEN_LOCATION, theaterOpenLocationPrivate))

G_DEFINE_TYPE (theaterOpenLocation, theater_open_location, GTK_TYPE_DIALOG)

/* GtkBuilder callbacks */
G_MODULE_EXPORT void uri_entry_changed_cb (GtkEditable *entry, GtkDialog *dialog);

static void
theater_open_location_class_init (theaterOpenLocationClass *klass)
{
	g_type_class_add_private (klass, sizeof (theaterOpenLocationPrivate));
}

static void
theater_open_location_init (theaterOpenLocation *self)
{
	GtkBuilder *builder;

	self->priv = theater_OPEN_LOCATION_GET_PRIVATE (self);
	builder = theater_interface_load ("uri.ui", FALSE, NULL, self);
...
static gboolean
theater_open_location_match (GtkEntryCompletion *completion, const gchar *key, GtkTreeIter *iter, gpointer user_data)
{
...
static gint
theater_compare_recent_stream_items (GtkRecentInfo *a, GtkRecentInfo *b)
{
...
char *
theater_open_location_get_uri (theaterOpenLocation *open_location)
{
	char *uri;

	g_return_val_if_fail (theater_IS_OPEN_LOCATION (open_location), NULL);
...
static char *
theater_open_location_set_from_clipboard (theaterOpenLocation *open_location)
{
...
GtkWidget *
theater_open_location_new (void)
{
	...
	open_location = theater_OPEN_LOCATION (g_object_new (theater_TYPE_OPEN_LOCATION,
	...
	clipboard_location = theater_open_location_set_from_clipboard (open_location);
	...
	/* Add items in theater's GtkRecentManager to the URI GtkEntry's GtkEntryCompletion */
	...
	if (recent_items != NULL)
	{
		...
		/* Filter out non-theater items */
		for (p = recent_items; p != NULL; p = p->next)
		{
			GtkRecentInfo *info = (GtkRecentInfo *) p->data;
			if (!gtk_recent_info_has_group (info, "theaterStreams")) {
		...
		streams_recent_items = g_list_sort (streams_recent_items, (GCompareFunc) theater_compare_recent_stream_items);
	...
	gtk_entry_completion_set_match_func (completion, (GtkEntryCompletionMatchFunc) theater_open_location_match, model, NULL);
