...
/**
 * SECTION:theater-plugin
 * @short_description: base plugin class and loading/unloading functions
 * @stability: Unstable
 * @include: theater-dirs.h
 *
 * libpeas is used as a general-purpose architecture for adding plugins to theater, with
 * derived support for different programming languages.
 *
 * The functions in theater-dirs.h are used to allow plugins to find and load files installed alongside the plugins, such as UI files.
 **/
...
/**
 * theater_get_plugin_paths:
 *
 * Return a %NULL-terminated array of paths to directories which can contain theater plugins. This respects the GSettings disable_user_plugins setting.
 *
...
theater_get_plugin_paths (void)
{
	...
	if (g_settings_get_boolean (settings, "disable-user-plugins") == FALSE) {
		path = g_build_filename (theater_data_dot_dir (), "plugins", NULL);
		g_ptr_array_add (paths, path);
	}
	...
	if (uninstalled == FALSE) {
		path = g_strdup (theater_PLUGIN_DIR);
		g_ptr_array_add (paths, path);
	}
...
/**
 * theater_plugin_find_file:
 * @plugin_name: the plugin name
 * @file: the file to find
 *
 * Finds the specified @file by looking in the plugin paths
 * listed by theater_get_plugin_paths() and then in the system
 * theater data directory.
 *
 ...
 **/
char *
theater_plugin_find_file (const char *plugin_name,
			const char *file)
{
	...
	engine = theater_plugins_engine_get_default (NULL);
	...
	/* global data files */
	if (ret == NULL)
		ret = theater_interface_get_full_path (file);
 ...
 **/
GtkBuilder *
theater_plugin_load_interface (const char *plugin_name,
			     const char *name,
			     gboolean fatal,
			     GtkWindow *parent,
			     gpointer user_data)
{
	...
	filename = theater_plugin_find_file (plugin_name, name);
	builder = theater_interface_load_with_full_path (filename,
						       fatal,
						       parent,
						       user_data);
	g_free (filename);
